<script>
  ...
  import { authStore, authHandlers } from "../../store";

  let showMenu = false;
  let showProfileMenu = false;
  const navigationLinks = [
    { text: "Dashboard", href: "/dashboard" },
    { text: "Compaigns", href: "/compaigns" },
    { text: "Managers", href: "/managers" },
  ...
  /**
   * @type {{ photoURL: string | null | undefined; displayName: any; email: any; } | null}
   */
  let currentUser;
  authStore.subscribe((value) => (currentUser = value.user));
</script>

<nav class="border-b border-gray-200 bg-white" data-sveltekit-reload>
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <div class="flex h-16 justify-between">
      <div class="flex">
        <div class="flex flex-shrink-0 items-center">
          <span class="text-xl font-semibold">AD</span>
        </div>
        <div class="hidden sm:-my-px sm:ml-6 sm:flex sm:space-x-8">
          {#each navigationLinks as link (link.href)}
            <a
              href={link.href}
              class={link.href === currentPath
          ...
          <div>
            ...
            >
              ...
              <img
                ...
                src={currentUser ? currentUser?.photoURL : "/profile.webp"}
                alt=""
              />
            </button>
          </div>
      ...
      <div class="-mr-2 flex items-center sm:hidden">
        ...
        <button
          ...
          type="button"
          class="relative inline-flex items-center justify-center rounded-md bg-white p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none"
        ...
        >
          ...
          <svg
            class="{showMenu ? 'hidden' : 'block'} h-6 w-6"
            fill="none"
            viewBox="0 0 24 24"
            stroke-width="1.5"
            stroke="currentColor"
          ...
          <svg
            class="{showMenu ? 'block' : 'hidden'} h-6 w-6"
            fill="none"
            viewBox="0 0 24 24"
            stroke-width="1.5"
            stroke="currentColor"
